...
We train a TransformerXL model on WikiText103 dataset using default hyperparameters (sequnece length = 224, num layers = 9, hidden size = 512, w/ whole-word tokenization and vocabulary size around 270k, which is the largest model that can be trained on 11 GB of GPU memory), giving rise to perplexity 20.43 and 21.61 on the validation and test set, respectively. 
